...
public class Player {
    ...
    public GameState play(final GameState gameState, final Deadline deadline) {
        ...
        //long criteria=deadline.timeUntil();
        //System.err.println("criteria "+criteria);
        //while(criteria>194168000 && d<7){
        //for(int j=0; j<nextStates.size();j++){
          // vi gar igenom alla mojliga forsta rotnoder. dvs alla forsta utlagg av x
          //System.err.println("kom hit 2");
          //double stateVal = miniMax(nextStates.elementAt(j), gameState.getNextPlayer());
          //System.err.println("kom hit 1"+stateVal);

          double stateVal = alphaBetaPruning(gameState,alpha,beta, gameState.getNextPlayer(),d);

          // if(stateVal>bestScore){
          //   bestindex=j;
          //   bestScore=stateVal;
      ...
      ///nextStates.elementAt(j),alpha,beta, gameState.getNextPlayer());
        ...
        if(d==0|| currState.isEOG()){//d==1){//if d=0 or node is a terminal node is has no next states     //currState.isEOG()){//d == 0 || nextStates.isEmpty()){ /// prova; currState.isEOG()
          ...
          if(player ==Constants.CELL_X){ //Maxes turn
            ...
            if(v>topScore) topScore= v;bestMove=cNode;
    ...
    public int evalv3(GameState state){
      ...
      for (int i =0; i<noPosLineUps; i++){
        ...
        for (int j = 0; j<lengthLineUp; j++){
          ...
          if (state.at(indexCell) == 1) maximizer=maximizer+1;
          if (state.at(indexCell) == 2) minimizer=minimizer+1;
      }
